...
Class Action {
	...
	//*To delete Passengers Account
	function delete_account(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM cards where id = ".$id);
		if($delete)
			return 1;
	}

	//*To delete Jeep's plate number
	function delete_jeep(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM jeeps where id = ".$id);
		if($delete)
			return 1;
	}
	...
	function update_comment(){
		...
		$chk = $this->db->query("SELECT * FROM jeeps where comment = '$comment'")->num_rows;

		if($chk > 0){
			return 2;
			exit;
		}
			$save = $this->db->query("UPDATE jeeps set $data where comment = '$comment' ");
	}
	...
	function save_jeep(){
		...
		$chk = $this->db->query("Select * from jeeps where station = '$station' and id !='$id' ")->num_rows;
		if($chk > 0){
			return 2;
			
		}
		else if(empty($id)){
			$save = $this->db->query("INSERT INTO jeeps set ".$data);
		}else{
			$save = $this->db->query("UPDATE jeeps set ".$data." where id = ".$id);
		}
		if($save){
			return 1;
		}
	...
	function get_price(){
		...
		$get['senior_price'] = isset($get['senior_price']) ? $get['senior_price'] : 0;
	...
	function save_driverInfo(){
		extract($_POST);
		$driverData = " driver_accountNo = '$driverAccountNo'";
		$driverData .= ", driver_name = '$driverName' ";
		$driverData .= ", driver_mobileNum = '$driverMobile'";
		$driverData .= ", driver_email = '$driverEmail'";

		$chk = $this->db->query("Select * from drivers_tbl where driver_accountNo = '$driverAccountNo' and 
									driver_name = '$driverName' and
									driver_mobileNum = '$driverMobile' and 
									driver_email = '$driverEmail' and
									id !='$id' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		else if(empty($id)){
			$save = $this->db->query("INSERT INTO drivers_tbl set ".$driverData);
		}else{
			$save = $this->db->query("UPDATE drivers_tbl set ".$driverData." where id = ".$id);
		}
		if($save){
			return 1;
		}
	...
	function delete_driverInfo(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM drivers_tbl where id = ".$id);
		if($delete)
			return 1;
	}
